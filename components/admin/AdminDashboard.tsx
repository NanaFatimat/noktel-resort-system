      ...
      if (currentUser) {
        try {
          import('firebase/firestore').then(async ({ doc, getDoc }) => {
             ...
             if (newRole !== 'admin') {
               // Log out non-admins trying to access admin dashboard
               await signOut(auth);
               setAuthError("You do not have administration privileges.");
             }
    ...
    try {
      if (isLoginMode) {
        ...
        if (inviteCode) {
          ...
          // Normal customer registration block removed from Admin Dashboard
          throw new Error("Admin invite code is required to register here.");
        }
      }
    } catch (err: any) {
